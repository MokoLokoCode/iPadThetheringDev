...
| W-021 | M1 | Fujifilm observational diagnostic probe | W-020 / F-G0 | Build/install succeeds; bounded logs distinguish discovery/session/catalog/items/PTP/errors/restrictions; capture markers and log export; safe idempotent start/stop; no downloads/writes/shutter commands | In progress | Physical iPad discovery of X-T4 over USB reported twice (RUN-20260924-F2); session-only probe proposed for next hardware test. Catalog/file/PTP callbacks remain pending. |
| W-022 | M1 | X-T4 USB CARD READER visibility control | W-021 | F-G1: real device/session/catalog and known filenames observed; one idle disconnect/reconnect recorded | In progress | USB camera discovery passed, but the mode during the reported run and session/catalog evidence are unrecorded (RUN-20260924-F2). |
