...
| W-021 | M1 | Fujifilm observational diagnostic probe | W-020 / F-G0 | Build/install succeeds; bounded logs distinguish discovery/session/catalog/items/PTP/errors/restrictions; capture markers and log export; safe idempotent start/stop; no downloads/writes/shutter commands | In progress | Physical iPad USB discovery and session open reported (RUN-20260924-F2/F3). Catalog/item/PTP logging authored for next test; capture marker and hardware evidence remain pending. |
| W-022 | M1 | X-T4 USB CARD READER visibility control | W-021 | F-G1: real device/session/catalog and known filenames observed; one idle disconnect/reconnect recorded | In progress | Session and removal/reconnect observed, but X-T4 USB mode, catalog and known-file comparison not recorded (RUN-20260924-F3). |
